...
function getCombinedChartData(scores, listArr) {
  ...
  scores.forEach((item) => { myDogRow[item.category] = item.score; });
  if (listArr.breedAvgList) {
    listArr.breedAvgList.forEach((item) => { breedAvgRow[item.category] = item.score; });
  }
  if (listArr.petTypeAvgList) {
    listArr.petTypeAvgList.forEach((item) => { petTypeAvgRow[item.category] = item.score; });
  }
...
export default function PetScoreChart({ petData, listArr }) {
  ...
  const chartData = useMemo(
    () => getCombinedChartData(petData?.scores, listArr),
    [petData, listArr]
  );
      ...
      <ChartArea>
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <Tooltip />
            <Legend />
            <Bar dataKey="TOTAL" fill="#5EC8A7" name="종합 점수" radius={[4, 4, 0, 0]} />
