...
function computeIsRtlScrollbarOnLeft() {
	...
	let res = outerEl.firstChild.getBoundingClientRect().left > outerEl.getBoundingClientRect().left;
...
function isInteractionValid(interaction, dateProfile, context) {
	...
	for (let instanceId in instances) if (!rangeContainsRange(dateProfile.validRange, instances[instanceId].range)) return false;
	...
	for (let subjectInstanceId in subjectInstances) {
		...
		for (let otherInstanceId in otherInstances) {
			...
			if (rangesIntersect(subjectRange, otherInstance.range)) {
				if (otherConfigs[otherInstance.defId].overlap === false && interaction.isEvent) return false;
...
function isDateSelectionPropsValid(state, context, dateSpanMeta, filterConfig) {
	...
	for (let selectionAllow of selectionConfig.allows) if (!selectionAllow(buildDateSpanApiWithContext(Object.assign(Object.assign({}, dateSpanMeta), selection), context), null)) return false;
	return true;
}
function allConstraintsPass(constraints, subjectRange, otherEventStore, businessHoursUnexpanded, context) {
	for (let constraint of constraints) if (!anyRangesContainRange(constraintToRanges(constraint, subjectRange, otherEventStore, businessHoursUnexpanded, context), subjectRange)) return false;
	return true;
}
...
function anyRangesContainRange(outerRanges, innerRange) {
	for (let outerRange of outerRanges) if (rangeContainsRange(outerRange, innerRange)) return true;
	return false;
}
...
var CalendarDataManager = class {
	...
	computeOptionsData(optionOverrides, dynamicOptionOverrides, calendarApi) {
		...
		return this.stableCalendarOptionsData = {
			...
			viewSpecs,
			theme,
			toolbarConfig: this.parseToolbars(refinedOptions, this.stableOptionOverrides, theme, viewSpecs, calendarApi),
			localeDefaults,
...
//#endregion
export { diffPoints as $, WindowScrollController as A, isPropsEqual as At, buildEventRangeKey as B, removeElement as Bt, ScrollController as C, interactionSettingsToStore as Ct, StandardEvent as D, isDateSelectionValid as Dt, Slicer as E, intersectSpans as Et, allowSelection as F, pointInsideRect as Ft, computeRect as G, startOfDay as Gt, buildSegTimeText as H, renderScrollShim as Ht, applyMutationToEventStore as I, preventContextMenu as It, createDuration as J, _ as Jt, config as K, triggerDateSelect as Kt, applyStyle as L, preventSelection as Lt, addMs as M, memoize as Mt, addWeeks as N, parseDragMeta as Nt, ViewContainer as O, isDateSpansEqual as Ot, allowContextMenu as P, parseEventDef as Pt, diffDates as Q, buildEntryKey as R, rangeContainsRange as Rt, RefMap as S, interactionSettingsStore as St, SimpleScrollGrid as T, intersectRects as Tt, compareNumbers as U, setRef as Ut, buildNavLinkAttrs as V, renderFill as Vt, computeInnerRect as W, sortEventSegs as Wt, createEventInstance as X, y as Xt, createEmptyEventStore as Y, d as Yt, createFormatter as Z, EventImpl as _, getStickyHeaderDates as _t, BgEvent as a, eventTupleToStore as at, NowTimer as b, identity as bt, DateProfileGenerator as c, getClippingParents as ct, DaySeriesModel as d, getEventTargetViaRoot as dt, diffWeeks as et, DayTableModel as f, getRectCenter as ft, EventContainer as g, getStickyFooterScrollbar as gt, Emitter as h, getSegMeta as ht, BaseComponent as i, enableCursor as it, addDays as j, mapHash as jt, WeekNumberContainer as k, isInteractionValid as kt, DayCellContainer as l, getDefaultEventEnd as lt, ElementScrollController as m, getSegAnchorAttrs as mt, createPlugin as n, elementClosest as nt, CustomRenderingStore as o, formatDayString as ot, ElementDragging as p, getRelevantEvents as pt, constrainPoint as q, whenTransitionDone as qt, BASE_OPTION_DEFAULTS as r, elementMatches as rt, DateComponent as s, formatIsoMonthStr as st, Calendar as t, disableCursor as tt, DayHeader as u, getElSeg as ut, Interaction as v, getUniqueDomId as vt, SegHierarchy as w, intersectRanges as wt, PositionCache as x, injectStyles as xt, MoreLinkContainer as y, hasCustomDayCellContent as yt, buildEventApis as z, refineEventDef as zt };

//# sourceMappingURL=internal-B4dhsDjx.js.map