import { $ as diffPoints, A as WindowScrollController, Bt as removeElement, C as ScrollController, Ct as interactionSettingsToStore, Dt as isDateSelectionValid, F as allowSelection, Ft as pointInsideRect, G as computeRect, Gt as startOfDay, I as applyMutationToEventStore, It as preventContextMenu, J as createDuration, K as config, Kt as triggerDateSelect, L as applyStyle, Lt as preventSelection, Nt as parseDragMeta, Ot as isDateSpansEqual, P as allowContextMenu, Pt as parseEventDef, Q as diffDates, Rt as rangeContainsRange, St as interactionSettingsStore, Tt as intersectRects, U as compareNumbers, W as computeInnerRect, X as createEventInstance, Y as createEmptyEventStore, _ as EventImpl, at as eventTupleToStore, bt as identity, ct as getClippingParents, dt as getEventTargetViaRoot, ft as getRectCenter, h as Emitter, it as enableCursor, jt as mapHash, kt as isInteractionValid, lt as getDefaultEventEnd, m as ElementScrollController, n as createPlugin, nt as elementClosest, p as ElementDragging, pt as getRelevantEvents, q as constrainPoint, qt as whenTransitionDone, r as BASE_OPTION_DEFAULTS, rt as elementMatches, tt as disableCursor, ut as getElSeg, v as Interaction, z as buildEventApis, zt as refineEventDef } from "./internal-B4dhsDjx.js";
...
var PointerDragging = class {
	...
	querySubjectEl(ev) {
		if (this.selector) return elementClosest(ev.target, this.selector);
	...
	destroyScrollWatch() {
		if (this.shouldWatchScroll) window.removeEventListener("scroll", this.handleScroll, true);
	}
...
function listenerCreated() {
	listenerCnt += 1;
	if (listenerCnt === 1) window.addEventListener("touchmove", onWindowTouchMove, { passive: false });
}
function listenerDestroyed() {
	listenerCnt -= 1;
	if (!listenerCnt) window.removeEventListener("touchmove", onWindowTouchMove, { passive: false });
}
...
var ElementMirror = class {
	...
	stop(needsRevertAnimation, callback) {
		...
		if (needsRevertAnimation && this.mirrorEl && this.isVisible && this.revertDuration && (this.deltaX || this.deltaY)) this.doRevertAnimation(done, this.revertDuration);
	...
	updateElPosition() {
		if (this.sourceEl && this.isVisible) applyStyle(this.getMirrorEl(), {
...
var OffsetTracker = class {
	...
	isWithinClipping(pageX, pageY) {
		...
		for (let scrollCache of this.scrollCaches) if (!isIgnoredClipping(scrollCache.getEventTarget()) && !pointInsideRect(point, scrollCache.clientRect)) return false;
		return true;
	}
};
	...
	processFirstCoord(ev) {
		...
		if (initialHit) {
			if (this.useSubjectCenter && subjectRect) {
				...
				if (slicedSubjectRect) adjustedPoint = getRectCenter(slicedSubjectRect);
			}
	...
	constructor(settings) {
		...
		this.handleHitUpdate = (hit, isFinal) => {
			...
			if (hit) {
				...
				if (!(hit.componentId === initialHit.componentId && this.isHitComboAllowed && !this.isHitComboAllowed(initialHit, hit))) dragSelection = joinHitsIntoSelection(initialHit, hit, context.pluginHooks.dateSelectionTransformers);
			...
			if (!isFinal) this.dragSelection = dragSelection;
		};
...
var EventDragging = class EventDragging extends Interaction {
	constructor(settings) {
		super(settings);
		this.subjectEl = null;
		...
		this.handlePointerDown = (ev) => {
			...
			let eventInstanceId = (this.eventRange = (this.subjectSeg = getElSeg(ev.subjectEl)).eventRange).instance.instanceId;
			this.relevantEvents = getRelevantEvents(initialContext.getCurrentData().eventStore, eventInstanceId);
			dragging.minDistance = ev.isTouch ? 0 : options.eventDragMinDistance;
			dragging.delay = ev.isTouch && eventInstanceId !== component.props.eventSelection ? getComponentTouchDelay(component) : null;
			...
			let isValid = component.isValidSegDownEl(origTarget) && !elementClosest(origTarget, ".fc-event-resizer");
	...
	displayDrag(nextContext, state) {
		...
		if (prevContext && prevContext !== nextContext) if (prevContext === initialContext) prevContext.dispatch({
			type: "SET_EVENT_DRAG",
			state: {
	...
	clearDrag() {
		...
		if (receivingContext) receivingContext.dispatch({ type: "UNSET_EVENT_DRAG" });
...
function computeEventMutation(hit0, hit1, eventInstanceStart, massagers) {
	...
	if (dateSpan0.allDay !== dateSpan1.allDay) {
		...
		if (dateSpan1.allDay) date0 = startOfDay(eventInstanceStart);
...
var EventResizing = class extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			let { component } = this;
			let eventRange = this.eventRange = getElSeg(this.querySegEl(ev)).eventRange;
		...
		this.handleHitUpdate = (hit, isFinal, ev) => {
			...
			if (hit) {
				if (!(hit.componentId === initialHit.componentId && this.isHitComboAllowed && !this.isHitComboAllowed(initialHit, hit))) mutation = computeMutation(initialHit, hit, ev.subjectEl.classList.contains("fc-event-resizer-start"), eventInstance.range);
			}
			if (mutation) {
...
function computeMutation(hit0, hit1, isFromStart, instanceRange) {
	...
	if (isFromStart) {
		if (dateEnv.add(instanceRange.start, delta) < instanceRange.end) return { startDelta: delta };
...
var OPTION_REFINERS = { fixedMirrorParent: identity };
...
var ExternalElementDragging = class {
	...
	buildDragMeta(subjectEl) {
		if (typeof this.suppliedDragMeta === "object") return parseDragMeta(this.suppliedDragMeta);
		if (typeof this.suppliedDragMeta === "function") return parseDragMeta(this.suppliedDragMeta(subjectEl));
		return getDragMetaFromEl(subjectEl);
	}
	displayDrag(nextContext, state) {
		let prevContext = this.receivingContext;
		if (prevContext && prevContext !== nextContext) prevContext.dispatch({ type: "UNSET_EVENT_DRAG" });
		if (nextContext) nextContext.dispatch({
			type: "SET_EVENT_DRAG",
			state
		});
	...
	clearDrag() {
		if (this.receivingContext) this.receivingContext.dispatch({ type: "UNSET_EVENT_DRAG" });
	}
...
function getDragMetaFromEl(el) {
	...
	return parseDragMeta(str ? JSON.parse(str) : { create: false });
}
...
var InferredElementDragging = class extends ElementDragging {
	constructor(containerEl) {
		...
		this.handlePointerDown = (ev) => {
			this.emitter.trigger("pointerdown", ev);
			if (!this.shouldIgnoreMove) this.emitter.trigger("dragstart", ev);
		};
		...
		this.handlePointerUp = (ev) => {
			this.emitter.trigger("pointerup", ev);
			if (!this.shouldIgnoreMove) this.emitter.trigger("dragend", ev);
		};
		let pointer = this.pointer = new PointerDragging(containerEl);
